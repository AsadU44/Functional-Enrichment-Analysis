...
### Read data
GENEs<- data.frame(read.csv('hub.csv'))

### Convert Symbol to ENTREZID
Converted <- bitr(unique(GENEs$Common), fromType = "SYMBOL",
                  toType = c("ENTREZID"),
                  OrgDb = org.Hs.eg.db) 
head(Converted)
...
dgn_res <- setReadable(All_result, 'org.Hs.eg.db', 'ENTREZID')
...
#### Cnet plot
Cnet_plot <- cnetplot(dgn_res, circular = TRUE,
                      showCategory=10, colorEdge = TRUE)
Cnet_plot
ggsave('cnet_plot.jpg', width = 32, height = 22, units = 'cm', bg='white')
